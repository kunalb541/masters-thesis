...
\begin{figure}[htbp]
    \centering
    \fbox{\textit{[Figure 2.1: Left panel shows the lens plane geometry with source trajectory crossing at impact parameter $u_0$, Einstein ring of radius $\theta_E$, and the two image positions. Right panel shows the corresponding symmetric PSPL light curve with magnification vs time, clearly labeled with $t_0$, $t_E$, and peak magnification $A_{\max} = (u_0^2 + 2)/(u_0\sqrt{u_0^2 + 4})$. Include $u_0 = 0.1$ and $u_0 = 0.5$ examples to show high vs moderate magnification.]}}
    \caption{Point-source point-lens geometry and light curve. Left: Source trajectory (blue line) with impact parameter $u_0$ relative to the lens (black dot). Right: Resulting symmetric magnification pattern showing characteristic temporal profile with Einstein timescale $t_E$.}
...
\begin{figure}[htbp]
    \centering
    \fbox{\textit{[Figure 2.2: Three panels showing binary lens caustic geometries. LEFT: Close binary ($s=0.7$, $q=0.5$) with merged caustic structure. CENTER: Resonant ($s=1.0$, $q=0.1$) with central and planetary caustics. RIGHT: Wide separation ($s=1.5$, $q=0.01$) with distinct central and small planetary caustics. For each panel, show: caustics (red filled regions), critical curves (blue curves in inset), lens positions (black dots), and example source trajectory (dashed line). Ensure angles $\alpha$, $\beta$, $\theta$ are clearly distinguishable per supervisor feedback.]}}
...
\begin{figure}[htbp]
    \centering
    \fbox{\textit{[Figure 2.3: Three example binary light curves (vertical panels) each paired with its corresponding caustic geometry (horizontal panels). TOP ROW: Caustic crossing event showing source trajectory (blue line) crossing the central caustic, with corresponding light curve showing sharp spike superimposed on broader magnification peak. MIDDLE ROW: Near-caustic event with source passing close to but not crossing caustic, showing asymmetric perturbation in light curve. BOTTOM ROW: Low-magnification trajectory passing far from caustics, showing light curve nearly indistinguishable from PSPL. Use distinct parameters: $(s=0.8, q=0.3, u_0=0.1)$, $(s=1.1, q=0.1, u_0=0.2)$, $(s=1.0, q=0.01, u_0=0.5)$ respectively. Ensure geometry and light curve panels align horizontally to show the causal relationship.]}}
    \caption{Binary microlensing light curve morphologies and their caustic geometries. Each row shows a source trajectory (left) and the resulting light curve (right) for different binary parameters and impact parameters. Top: Caustic crossing produces sharp flux spike. Middle: Near-caustic approach creates asymmetric perturbation. Bottom: Distant trajectory yields nearly PSPL-like profile, illustrating the fundamental detection limit at large impact parameters.}
